						...
						if basicSetting[2] != '0':
							if int(basicSetting[17]) <= bossMungCnt[i] and int(basicSetting[17]) != 0:
								...
								################ 멍 보스 ################
									...
									tmp_bossTimeString[i] = bossTimeString[i] = nextTime.strftime('%H:%M:%S')
									tmp_bossDateString[i] = bossDateString[i] = nextTime.strftime('%Y-%m-%d')
									await client.get_channel(channel).send("```" + bossData[i][0] + ' 르야 미입력 됬어.```')
									embed = discord.Embed(
										description= '```다음 ' + bossData[i][0] + ' ' + bossTimeString[i] + '입니다.```',
										color=0xff0000
										)
...
async def on_ready():
	...
	await client.change_presence(status=discord.Status.onlin, activity=discord.Game(name='V4', type=1), afk=False)
...
while True:
	...
	async def setting_(ctx):	
		...
		if ctx.message.channel.id == basicSetting[7]:
			setting_val = '보탐봇버전 : Server Ver. 16.2 (2020. 4. 21.)\n'
